...
def consume(decision: Any, expected: ApplicabilityExpectation) -> dict[str, Any]:
    ...
    if not _valid_expectation(expected):
        return {"outcome": "cannot_establish", "reason": "invalid_expectation", "decision_identity": identity}
    ...
    try:
        ...
        return {"outcome": "cannot_establish", "reason": exc.code, "decision_identity": identity}
    ...
    if any(key not in effect["params"] or effect["params"][key] != value for key, value in requested.items()):
