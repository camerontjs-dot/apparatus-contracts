...
REGISTRY_PATH = Path(__file__).resolve().parents[1] / "schema" / "contract-d" / "1.0.0" / "effect-registry.json"
...
def _json_value(value: Any, path: str = "$", active: set[int] | None = None, container_depth: int = 1) -> None:
    ...
    if isinstance(value, list):
        if container_depth > MAX_JSON_CONTAINER_DEPTH:
            raise ContractDError("json_depth_exceeded", path, f"max_container_depth={MAX_JSON_CONTAINER_DEPTH}")
        marker = id(value)
        if marker in active:
