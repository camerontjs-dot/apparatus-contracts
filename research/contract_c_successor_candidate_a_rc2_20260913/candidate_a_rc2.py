...
_spec = importlib.util.spec_from_file_location("candidate_a_rc1_parent_for_rc2", _RC1_PATH)
if _spec is None or _spec.loader is None:
    raise RuntimeError(f"cannot load exact Candidate A RC1 parent: {_RC1_PATH}")
RC1 = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = RC1
    ...
    if isinstance(props, list):
        for prop in props:
            if not isinstance(prop, dict):
                continue
            terminal = prop.get("terminal")
            if isinstance(terminal, dict) and terminal.get("reason") == UNSUPPORTED_REASON:
                terminal["reason"] = "no_deciding_relation"
    return RC1.seal(x)


def _validate_rc2_only_terminal(prop: Mapping[str, Any], reason: str | None, where: str) -> None:
    ...
    _require(execution.get("state") == "completed", f"{where}: unsupported family requires completed execution")
    _require(execution.get("completion") == "not_checkable", f"{where}: unsupported family requires not_checkable completion")
    _require(isinstance(terminal, dict), f"{where}.terminal: expected object")
    _require(terminal.get("verdict") == "not_checkable", f"{where}: unsupported family requires not_checkable verdict")
    _require(groups == [], f"{where}: unsupported family cannot carry causal basis groups")
    _require(isinstance(participants, list), f"{where}.participants: expected array")
    for index, participant in enumerate(participants):
        _require(isinstance(participant, dict), f"{where}.participants[{index}]: expected object")
        _require(
...
def validate_object(value: Mapping[str, Any]) -> None:
    ...
    _require(value["profile"] == PROFILE, "candidate.profile: wrong/unknown RC2 research profile")
    ...
    for index, (prop, reason) in enumerate(_reason_rows(value)):
        if reason is not None:
            _require(reason in PUBLIC_REASONS, f"candidate.propositions[{index}].terminal.reason: unknown public reason")
