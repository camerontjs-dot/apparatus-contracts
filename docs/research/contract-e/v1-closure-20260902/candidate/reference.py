...
RECEIPT_SCHEMA = "contract-e-authorization-receipt-candidate-rc1"
...
def strict_json_loads(text: str) -> Any:
    ...
    value = json.loads(
        ...
        parse_constant=lambda token: (_ for _ in ()).throw(InvalidCanonicalJSON(f"invalid constant: {token}")),
    )
    _assert_finite_json(value)
    return value
...
def validate_authority_state(state: Any) -> tuple[bool, list[str]]:
    ...
    if not _exact_keys(state, STATE_KEYS) or state.get("schema") != STATE_SCHEMA:
        return False, ["malformed_state"]
    if not _nonempty_string(state.get("authority_state_id")):
        return False, ["malformed_state"]
...
def _validate_references(references: Any) -> tuple[bool, set[str]]:
    ...
    for ref in references:
        if not _exact_keys(ref, REFERENCE_KEYS):
            return False, set()
        if not all(_nonempty_string(ref[k]) for k in ("ref_id", "kind", "immutable_id", "identity_sha256")):
            return False, set()
        ...
        if ref["ref_id"] in ids:
            return False, set()
        ids.add(ref["ref_id"])
        if ref["identity_sha256"] != reference_identity(ref["kind"], ref["version"], ref["immutable_id"]):
            return False, set()
    return True, ids
...
def validate_request(request: Any) -> tuple[bool, list[str]]:
    ...
    if not _exact_keys(request, REQUEST_KEYS) or request.get("schema") != REQUEST_SCHEMA:
        return False, ["malformed_request"]
    for key in ("request_id", "authority_state_id", "evaluation_time", "subject_id"):
    ...
    if not _exact_keys(jurisdiction, JURISDICTION_KEYS) or not all(_nonempty_string(jurisdiction[k]) for k in JURISDICTION_KEYS):
        return False, ["malformed_request"]
...
def evaluate(authority_state: Any, request: Any) -> dict:
    ...
    receipt = {
        ...
        "authority_state_id": authority_state.get("authority_state_id") if isinstance(authority_state, dict) else None,
        "evaluation_time": request.get("evaluation_time") if isinstance(request, dict) else None,
        "subject_id": request.get("subject_id") if isinstance(request, dict) else None,
        "jurisdiction": deepcopy(request.get("jurisdiction")) if isinstance(request, dict) and isinstance(request.get("jurisdiction"), dict) else None,
