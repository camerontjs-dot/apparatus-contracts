...
def _basis_complete_and_conferring(basis):
...
def _validate_receipt(case, rid, memo, stack):
    if rid in memo:
        return memo[rid]
    if rid in stack:
...
def _validate_deps(case, node, memo, stack):
...
def _validate_node(case, node, memo, stack):
...
def _authorized_resolutions(case, request, memo):
    resolved = set()
    for rid in request.get("resolver_receipt_ids", []):
        ok, _ = _validate_receipt(case, rid, memo, set())
        if ok:
            receipt = _idx(case)[rid]
            if receipt.get("authority_kind") == "resolution":
                resolved.update(receipt.get("resolves_ids", []))
    return resolved
