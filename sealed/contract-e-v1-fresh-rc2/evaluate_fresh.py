...
def compare_callable(observed_evaluate, reference_evaluate):
    records = []
    false_permits = []
    false_rejects = []
    exceptions = []
    ...
    for item in cases():
        expected = reference_evaluate(deepcopy(item["state"]), deepcopy(item["request"]))
        error = None
        try:
            observed = observed_evaluate(deepcopy(item["state"]), deepcopy(item["request"]))
        ...
        expected_allowed = isinstance(expected, dict) and expected.get("authorized") is True
        ...
        dual_identity_ok = isinstance(observed, dict) and (
            observed.get("claimed_authority_state_id") == expected.get("claimed_authority_state_id")
            and observed.get("recomputed_authority_state_id") == expected.get("recomputed_authority_state_id")
        )
        if not dual_identity_ok:
        ...
        records.append({
            ...
            "dual_identity_ok": dual_identity_ok,
            "exception": error,
        })

    count = len(records)
    result = {
        "schema": "contract-e-v1-fresh-rc2-comparison-v1",
        "case_count": count,
        "normative_exact_matches": count - len(normative_mismatches),
    ...
    result["scientific_state"] = (
        ...
        and not dual_identity_failures
        else "FALSIFIED"
    )
    return result
...
def compare(implementation_path: Path):
    reference = load_module(REFERENCE_PATH, "contract_e_rc2_reference")
    implementation = load_module(implementation_path, "contract_e_rc2_fresh_impl")
    ...
    return compare_callable(implementation.evaluate, reference.evaluate)
