...
NORMATIVE_FIELDS = [
    "schema", "receipt_id", "authority_conferring", "authorized", "request_id",
    "request_sha256", "authority_state_id", "evaluation_time", "subject_id",
    "jurisdiction", "authority_basis_id", "preserved",
]
...
def compare(implementation_path: Path):
    reference = load_module(REFERENCE_PATH, "contract_e_v1_reference")
    implementation = load_module(implementation_path, "contract_e_v1_fresh_impl")
    ...
    for item in cases():
        ...
        diagnostic_shape_ok = isinstance(observed, dict) and isinstance(observed.get("diagnostics"), list) and all(isinstance(x, str) for x in observed.get("diagnostics", []))
    ...
    result = {
        "schema": "contract-e-v1-fresh-comparison-v1",
        "case_count": count,
        ...
        "records": records,
        "scientific_state": "SUPPORTED" if not normative_mismatches and not exceptions and not preservation_failures and not diagnostic_shape_failures else "FALSIFIED",
    }
    return result
